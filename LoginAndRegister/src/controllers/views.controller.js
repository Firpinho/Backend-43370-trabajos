...
const getAll = async (req, res, next) => {
    try {
        const cartID = await cartCookie(req, res);
        const { limit, page, sort, query } = req.query
        const response = await productServices.getAllService(limit, page, sort, query);
        const products = response.docs;
        res.status(200).render('home', { data: { products: products, cart: cartID , user: req.session.name} });
    } catch (error) {
        next(error.message);
    }
}
...
const cartCookie = async (req, res) => {
